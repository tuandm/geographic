...
class Database {
	...
	public function __construct($config = array())
	{
		...
		if (is_string($this->config['connection']))
		{
			...
			if (strpos($this->config['connection'], '://') === FALSE)
				throw new \Dit\Database\Exception('database.invalid_dsn', $this->config['connection']);
		...
		// Set driver name
		$driver = '\\Dit\\Database\\Driver\\' . ucfirst($this->config['connection']['type']);
	...
	public function connect()
	{
		...
		if (!is_resource($this->link) AND ! is_object($this->link))
		{
			...
			if (!is_resource($this->link) AND ! is_object($this->link))
				throw new \Dit\Database\Exception('database.connection', $this->driver->show_error());
	...
	public function insert($table = '', $set = NULL)
	{
		...
		if ($this->set == NULL)
			throw new \Dit\Database\Exception('database.must_use_set');

		if ($table == '')
		{
			if (!isset($this->from[0]))
				throw new \Dit\Database\Exception('database.must_use_table');
	...
	public function delete($table = '', $where = NULL)
	{
		...
		if (count($this->where) < 1)
			throw new \Dit\Database\Exception('database.must_use_where');
    ...
    public function get_last_insert_id() {
        $lastInsert = $this->query('select last_insert_id() as last_id')->current();
        return $lastInsert['last_id'];
    }
} // End Database Class
