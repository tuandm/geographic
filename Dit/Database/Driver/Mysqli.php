...
class Mysqli extends \Dit\Database\Driver\Mysql {
	...
	public function connect()
	{
		...
		if ($this->link = @new \mysqli($host, $user, $pass, $database, $port))
		{
			if(mysqli_connect_error()) {
				$this->link = null; 
				throw new \Dit\Database\Exception('database.connection', mysqli_connect_error());
			}
	...
	public function query($sql)
	{
		...
		if ($this->db_config['cache'] AND ! preg_match('#\b(?:INSERT|UPDATE|REPLACE|SET)\b#i', $sql))
		{
			...
			if ( ! isset(self::$query_cache[$hash]))
			{
				...
				self::$query_cache[$hash] = new \Dit\Database\Driver\Mysqli\Result($this->link, $this->db_config['object'], $sql);
			}
		...
		return new \Dit\Database\Driver\Mysqli\Result($this->link, $this->db_config['object'], $sql);
	}
	...
	public function set_charset($charset)
	{
		...
		{
			//percona with outdated mysqli.so php extesion work via set only
			...
			if(!$query)
			{
				throw new \Dit\Database\Exception('database.error', $this->show_error());
			}
		}
	...
	public function stmt_prepare($sql = '')
	{
		...
		return new \Dit\Database\Driver\Mysqli\Statement($sql, $this->link);
	}
 ...
 * MySQLi Result
 */
namespace Dit\Database\Driver\Mysqli;
class Result extends \Dit\Database\Result {
	...
	public function __construct($link, $object = TRUE, $sql)
	{
		...
		{
			// SQL error
			throw new \Dit\Database\Exception('database.error', $this->link->error.' - '.$sql);
		}
		else
		{
