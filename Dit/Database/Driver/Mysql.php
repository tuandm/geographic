...
class Mysql extends \Dit\Database\Driver {
	...
	public function list_tables(\Dit\Database $db)
	{
		static $tables;
	...
	public function list_fields($table)
	{
		...
		if (!isset($tables[$table]))
			throw new \Dit\Database\Exception('database.table_not_found', $table);
 ...
 * MySQL Result
 */
namespace Dit\Database\Driver\Mysql;
class Result extends \Dit\Database\Result {
	...
	public function __construct($result, $link, $object = TRUE, $sql)
	{
		...
		elseif (is_bool($result))
		{
			...
			{
				// SQL error
				throw new \Dit\Database\Exception('database.error', mysql_error($link).' - '.$sql);
			}
			else
			{
